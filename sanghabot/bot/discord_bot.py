...
RAG_DISCLAIMER = "⚠️ This summary was generated by AI from the excerpts below and may contain mistakes. Please verify against the source excerpts."
...
class SanghaBot(discord.Client):
    ...
    async def perform_search(self, query: str, message: discord.Message, is_dm_private=False, is_dm_proxy=False):
        try:
            ...
            if answer_text:
                for block in self.split_text_for_embeds(answer_text):
                    await post_target.send(embed=discord.Embed(description=block, color=discord.Color.blurple()))
